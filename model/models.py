...
class BertClassifier(th.nn.Module):
    def __init__(self, pretrained_model='csebuetnlp/banglabert', nb_class=20):
        super(BertClassifier, self).__init__()
        self.nb_class = nb_class
        self.tokenizer = AutoTokenizer.from_pretrained(pretrained_model)
        model1 = BertForSequenceClassification.from_pretrained(pretrained_model, num_labels=nb_class)
        self.bert_model = AutoModel.from_pretrained(pretrained_model)
#        self.bert_model = AutoModel.from_pretrained(pretrained_model)
        self.feat_dim = model1.classifier.in_features
